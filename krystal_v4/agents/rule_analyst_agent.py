...
def create_rule_analyst_agent(llm=None) -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            "conditional logic, name parsing, string manipulation, substring extraction, and phone number parsing. "
            "\n\n"
            ...
            "7. **substring** - Extracts LEFT/RIGHT/MID substring (e.g., first 3 chars of phone → area code)\n"
            "8. **phone_parser** - Parses phone numbers to extract area_code or phone_number parts\n"
            "\n\n"
            "CRITICAL: You must ONLY use the transformation types listed above. "
            "The config dict for each type has a STRICT schema — do NOT invent config fields. "
            "The task description will provide the exact config schema for each type.\n\n"
            ...
            "- 'left 3 characters' or 'first 3 digits' → substring with method='left', length=3\n"
            "- 'area code from phone' → phone_parser with part='area_code'\n"
            "- 'phone number without area code' → phone_parser with part='phone_number'\n"
            "\n\n"
